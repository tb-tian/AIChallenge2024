...
from helpers import get_logger, is_exits
...
def video_to_audio(in_path, out_path):
    ...
    command = f"ffmpeg -i {in_path} -vf scale=360:-2 -c:v libx264 -crf 25 -preset medium -c:a copy {out_path}"
